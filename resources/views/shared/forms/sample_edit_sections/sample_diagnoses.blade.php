@php
    $diagnosis_action_route = $included_action_route ?? route("patients.samples.diagnoses");
...
{{-- <form action='{{ route("patients.stationary.update_many_diagnoses") }}' class="ajax-submitable dont-reset" method="POST"> --}}
            ...
            <div class="container">
                <div class="col-xs-12 my-2">
                    ...
                    <x-forms.magic-search class="magic-search ajax" data-catalog-name="diseases" value='{{ $item->disease_id }}'
                                          hidden-id="old_diagnoses_edit_{{ $item->id }}" value="{{ $item->id }}" hidden-name="disease_id"
...
<form action="{{$route_delete}}" method="POST" class="ajax-submitable" id='{{$delete_form_id}}'>
    @csrf
    <input type="hidden" name="diagnosis_id" value="{{$item->id ?? null}}">
    <input type="hidden" name="hide_form_id" value='{{$insert_form_id}}'>
</form>
