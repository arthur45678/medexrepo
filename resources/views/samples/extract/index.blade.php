...
<div class="card-header-actions">
    <a href="{{ route("samples.patients.extract.create", ['patient' => $patient]) }}" class="btn btn-primary float-right mr-4" type="button">
        <x-svg icon="cui-plus" />
        Նոր ՔԱՂՎԱԾՔ
    </a>
</div>
...
<table class="table table-striped table-bordered table-cursor datatable-default">
    ...
    <tbody>
        @foreach ($list as $im)
        <tr data-url="{{ route('samples.patients.extract.show', [$patient , $im]) }}">
            <td>
                {{ $im->id }}
            </td>
            <td>
            ...
            <td>
                <a href="{{ route('samples.patients.extract.show', [$patient , $im]) }}" class="btn btn-info btn-sm">
                    <x-svg icon="cui-external-link" />
                </a>
                @if($im->user_id==auth()->id())
                @can('belongs-to-user', $im)
                <a href="{{ route('samples.patients.extract.edit', [$patient->id , $im->id]) }}" class="btn btn-primary btn-sm">
                    <x-svg icon="cui-pencil" />
                </a>
                @endcan
                <form method="POST" action="{{route('samples.patients.extract.destroy', [$patient->id , $im->id]) }}" class="d-inline">
                    @csrf
                    @method('delete')
