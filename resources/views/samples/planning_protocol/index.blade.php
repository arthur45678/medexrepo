...
@section('card-header')
<div>
    <h4>ՊԼԱՆԱՎՈՐՄԱՆ ՈՒՂԵԳԻՐ</h4>
    <h5>{{ $patient->all_names }}</h5>
</div>
<div class="card-header-actions">
    <a href="{{ route("samples.patients.planning-protocol.create", ['patient' => $patient]) }}" class="btn btn-primary float-right mr-4" type="button">
        <x-svg icon="cui-plus" />
        Նոր ՊԼԱՆԱՎՈՐՈՒՄ
    </a>
</div>
...
<table class="table table-striped table-bordered table-cursor datatable-default">
    ...
    <tbody>
        @foreach ($planning as $im)
        <tr data-url="{{ route('samples.patients.planning-protocol.show', [$patient->id , $im->id]) }}">
            <td>
                {{ $im->id }}
            </td>
            <td>
            ...
            <td>
                <a href="{{ route('samples.patients.planning-protocol.show', [$patient->id , $im->id]) }}" class="btn btn-info btn-sm">
                    <x-svg icon="cui-external-link" />
                </a>
                @if($im->user_id==auth()->id())
                @can('belongs-to-user', $im)
                <a href="{{ route('samples.patients.planning-protocol.edit', [$patient->id , $im->id]) }}" class="btn btn-primary btn-sm">
                    <x-svg icon="cui-pencil" />
                </a>
                @endcan
                    <form method="POST" action="{{route('samples.patients.planning-protocol.destroy', [$patient->id , $im->id]) }}" class="d-inline">
                        @csrf
                        @method('delete')
