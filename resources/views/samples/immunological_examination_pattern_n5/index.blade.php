...
@section('card-header')
<div>
    <h4>ԻՄՈՒՆԱԲԱՆԱԿԱՆ ԼԱԲՈՐԱՏՈՐԻԱ ԲԺՇԿԱԿԱՆ ՁԵՎ N 5</h4>
    <h5>{{ $patient->all_names }}</h5>
</div>
<div class="card-header-actions">
    <a href="{{ route("samples.patients.iep-n5.create", ['patient' => $patient]) }}" class="btn btn-primary float-right mr-4" type="button">
        <x-svg icon="cui-plus" />
        Նոր հետազոտություն
    </a>
</div>
...
<table class="table table-striped table-bordered table-cursor datatable-default">
    ...
    <tbody>
        ...
        <tr data-url="{{ route('samples.patients.iep-n5.show', [$patient , $im->id]) }}">
            <td>
                {{ $im->id }}
            </td>
            <td>
            ...
            <td>
                <a href="{{ route('samples.patients.iep-n5.show', [$patient , $im->id]) }}" class="btn btn-info btn-sm">
                    <x-svg icon="cui-external-link" />
                </a>
                @if($im->user_id==auth()->id())
                @can('belongs-to-user', $im)
                <a href="{{ route('samples.patients.iep-n5.edit', [$patient->id , $im->id]) }}" class="btn btn-primary btn-sm">
                    <x-svg icon="cui-pencil" />
                </a>
                @endcan
                <form method="POST" action="{{route('samples.patients.iep-n5.destroy', [$patient->id , $im->id]) }}" class="d-inline">
                    @csrf
                    @method('delete')
