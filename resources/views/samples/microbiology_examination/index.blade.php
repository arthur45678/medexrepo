...
<div class="card-header-actions">
    <a href="{{ route("samples.patients.microbiology_examination.create", ['patient' => $patient, 'apse_list' => $apse_list]) }}" class="btn btn-primary float-right mr-4" type="button">
        <x-svg icon="cui-plus" />
...
<table class="table table-striped table-bordered table-cursor datatable-default">
    ...
    <tbody>
        ...
        <tr data-url="{{ route("samples.patients.microbiology_examination.show", ['patient' => $patient , $apse->id]) }}">
            ...
            <td>
                ...
                <a href="{{ route("samples.patients.microbiology_examination.show", ['patient' => $patient , $apse->id]) }}" class="btn btn-info btn-sm">
                    <x-svg icon="cui-external-link" />
                </a>

                @can('belongs-to-user', $apse)
                <a href="{{ route('samples.patients.microbiology_examination.edit', ['patient' => $patient, $apse->id]) }}" class="btn btn-primary btn-sm">
                ...
                @if($apse->user_id == Auth::user()->id)
                    <form method="POST" action="{{route('samples.patients.microbiology_examination.destroy',['patient' => $patient,$apse->id]) }}" class="d-inline">
                        @csrf
                        @method('delete')
