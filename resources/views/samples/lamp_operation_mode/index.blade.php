...
@section('card-header')
<div>
    <h4>ԼԱՄՊԻ ՇԱՀԱԳՈՐԾՄԱՆ ՌԵԺԻՄ</h4>
    <h5>{{ $patient->all_names }}</h5>
</div>
<div class="card-header-actions">
    <a href="{{ route("samples.patients.lamp-operation-mode.create", ['patient' => $patient]) }}" class="btn btn-primary float-right mr-4" type="button">
        <x-svg icon="cui-plus" />
  Ավելեցնել նորը
    </a>
    <a href="{{ route('samples.patients.lamp-operation-mode.show', [$patient->id ,0]) }}" class="btn btn-primary float-right mr-4">
...
<table class="table table-striped table-bordered table-cursor datatable-default">
    ...
    <tbody>
        @foreach ($lamp as $im)
        <tr data-url="{{ route('samples.patients.lamp-operation-mode.show', [$patient->id ,0]) }}">
            <td>
                {{ $im->id }}
            </td>
            <td>
            ...
            <td>
                ...
                <a href="{{ route('samples.patients.lamp-operation-mode.edit', [$patient->id , $im->id]) }}" class="btn btn-primary btn-sm">
                    <x-svg icon="cui-pencil" />
                </a>
                @endcan

                    <form method="POST" action="{{route('samples.patients.lamp-operation-mode.destroy', [$patient->id , $im->id]) }}" class="d-inline">
                        @csrf
                        @method('delete')
