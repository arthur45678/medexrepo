...
@section('card-header')
    ...
    <div class="card-header-actions">
        <a href="{{ route("admin.marital-status-lists.create") }}" class="btn btn-primary float-right mr-4" type="button" >
            <x-svg icon="cui-plus" />
            ավելացնել նորը
        </a>
    </div>
    ...
    <table class="table table-md table-hover table-responsive table-cursor datatable-default" style="width:100%;">
        ...
        <tbody>
            @foreach ($lists as $key => $list)
                <tr data-url='{{route("admin.marital-status-lists.edit", $list->id)}}'>
                    <td>{{ $list->id }}</td>
                    <td>{{ $list->name }}</td>
                    ...
                    @endif
                    <td>
                        <form method="POST" action='{{route("admin.marital-status-lists.update",  $list->id)}}' class="d-inline">
                            @csrf
                            @method("PATCH")
                        ...
                        <a href="{{route("admin.marital-status-lists.edit",  $list->id)}}" class="btn btn-primary btn-sm" target="_blank">
