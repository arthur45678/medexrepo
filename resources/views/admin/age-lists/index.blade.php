...
@section('card-header')
    ...
    <div class="card-header-actions">
        <a href="{{ route("admin.age-lists.create") }}" class="btn btn-primary float-right mr-4" type="button" >
            <x-svg icon="cui-plus" />
            ավելացնել նորը
        </a>
    </div>
        ...
        <tbody>
            @foreach ($lists as $key => $list)
                <tr data-url='{{route("admin.age-lists.edit", $list->id)}}'>
                    ...
                    @endif
                    <td>
                        <form method="POST" action='{{route("admin.age-lists.update",  $list->id)}}' class="d-inline">
                            @csrf
                            @method("PATCH")
                        ...
                        <a href="{{route("admin.age-lists.edit",  $list->id)}}" class="btn btn-primary btn-sm">
