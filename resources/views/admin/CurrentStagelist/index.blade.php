        ...
        <tbody>
            @foreach ($lists as $k=>$list)
                <tr>
                    ...
                    @endif
                    <td>
                        <form method="POST" action="{{route('admin.currentStage-lists.update',$list->id)}}" class="d-inline">
                            @csrf
                            @method("put")
                            <button class="btn btn-danger btn-sm" type="submit" name="{{$list->id}}">
                                <x-svg icon="cui-trash" />
                            </button>
                        </form>
                        <a href="{{route('admin.currentStage-lists.edit',$list->id)}}">
