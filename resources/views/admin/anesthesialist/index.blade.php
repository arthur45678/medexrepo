...
@section('card-header')
<h4>Անզգայացման տեսակներ</h4>
    <div class="card-header-actions">
        <a href="{{ route("admin.anesthesia-list.create") }}" class="btn btn-primary float-right mr-4" type="button" target="_blank">
            <x-svg icon="cui-plus" />
            ավելացնել նորը
        </a>
    </div>
        ...
        <tbody>
            @foreach ($lists as $list)
                <tr >
                    ...
                    @endif
                    <td>
                        <form method="POST" action="{{route('admin.anesthesia-list.update',$list->id)}}" class="d-inline">
                            @csrf
                            @method("put")
                        ...
                        <a href="{{route('admin.anesthesia-list.edit',$list->id)}}">
