...
@section('card-header')
    ...
    <div class="card-header-actions">
        <a href="{{ route("admin.researches-lists.create") }}" class="btn btn-primary float-right mr-4" type="button" target="_blank">
            <x-svg icon="cui-plus" />
            ավելացնել նորը
        </a>
    </div>
        ...
        <tbody>
            @foreach ($lists as $k=>$list)
                <tr >
                    ...
                    @endif
                    <td>
                       <form method="POST" action="{{route('admin.researches-lists.update',$list->id)}}" class="d-inline" >
                           @csrf
                           @method("put")
                        ...
                        <a href="{{route('admin.researches-lists.edit',$list->id)}}">
