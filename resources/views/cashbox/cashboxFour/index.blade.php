...
@section('card-content')
   ...
   @if(session('updated'))
    ...
    <x-modal modal-id="order-import-modal" title="Մուտքի օրդեր" form-id="order-import-form">
        <form action="{{ route('cashbox.cashboxFour.orderinput.store') }}" id="order-import-form" method="post">
            @csrf
    ...
    <x-modal modal-id="order-export-modal" title="Ելքի օրդեր" form-id="order-export-form">
        <form action="{{ route('cashbox.cashboxFour.orderoutput.store')  }}" id="order-export-form" method="post">
            @csrf
            ...
            <table id="example" class="table table-striped table-bordered table-cursor" style="width: 100%;">
                ...
                @foreach($orderOutput as $order)
                    <tr data-url={{url("/cashbox/order/{$order->id}/show_1_CashInputOrders")}}>
                        ...
                        <th>
                            <a class="btn btn-info btn-sm" href="{{ route('cashbox.cashboxFour.orderoutput.show', $order) }}">
                                <svg class="c-icon">
                                    <use xlink:href="/assets/icons/coreui/free-symbol-defs.svg#cui-applications"></use>
                                </svg>
                            </a>
                            <a href="{{ route('cashbox.cashboxFour.orderoutput.edit',$order->id) }}" class="btn btn-primary btn-sm" >
                                <x-svg icon="cui-pencil" />
                            </a>
                            <form style="display:inline" action="{{ route('cashbox.cashboxFour.orderoutput.destroy',$order->id) }}" method="post">
                                @csrf
                                @method('DELETE')
            ...
            <table id="example" class="table table-striped table-bordered table-cursor" style="width: 100%;">
                ...
                @foreach($ordersInput as $order)
                    <tr data-url={{url("/cashbox/order/{$order->id}/show_1_CashInputOrders")}}>
                        ...
                        <th>{{ $order->patient->f_name }}&nbsp;{{ $order->patient->l_name }}&nbsp;{{ $order->patient->p_name }}</th>
                        <th>
                            <a class="btn btn-info btn-sm" href="{{ route('cashbox.cashboxFour.orderinput.show', $order->id) }}">
                                <svg class="c-icon">
                                    <use xlink:href="/assets/icons/coreui/free-symbol-defs.svg#cui-applications"></use>
                                </svg>
                            </a>
                            <a href="{{ route('cashbox.cashboxFour.orderinput.edit',$order->id) }}" class="btn btn-primary btn-sm" >
                                <x-svg icon="cui-pencil" />
                            </a>
                            <form style="display:inline" action="{{ route('cashbox.cashboxFour.orderinput.destroy',$order->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger btn-sm" type="submit"><x-svg icon="cui-trash" /></button>
                            </form>
