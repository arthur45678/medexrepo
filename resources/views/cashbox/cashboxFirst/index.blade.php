...
@section('card-content')
   ...
   @if(session('updated'))
    ...
    <x-modal modal-id="order-import-modal" title="Մուտքի օրդեր" form-id="order-import-form">
        <form action="{{ route('cashbox.cashboxFirst.orderinput.store') }}" id="order-import-form" method="post"  >
            @csrf
    ...
    <x-modal modal-id="order-export-modal" title="Ելքի օրդեր" form-id="order-export-form">
        <form action="{{ route('cashbox.cashboxFirst.orderoutput.store')  }}" id="order-export-form" method="post">
            @csrf
            <div class="alert alert-success alert-block d-none">
                <button type="button" class="close" data-dismiss="alert">×</button>
                <strong class="alert-content"></strong>
            ...
            <div class="form-group">
                <label>Գումարը թվերով (Դրամ)</label>
                <x-forms.text-field  id="price_output" name="price" label="" validationType="ajax" />
            </div>

            <div class="form-group">
                <label>Գումարը տառերով (Դրամ)</label>
                <x-forms.text-field  id="sum_text_output" name="sum_text" label="" validationType="ajax" />
            </div>
            <div class="form-group">
                <label>Ստացողի անձը հաստատող փաստաթղթի անվանումը</label>
                <input name="document_type" type="text" class="form-control"  />
            ...
            <div class="form-group">
                ...
                <x-forms.text-field type="textarea" name="passport_data" placeholder="լրացման ազատ դաշտ․․․" class="mt-2"
                                    value="" label="" validationType="ajax" />
            </div>
            ...
            <table id="example" class="table table-striped table-bordered table-cursor" style="width: 100%;">
                ...
                @foreach($orderOutput as $order)
                    <tr data-url={{url("/cashbox/order/{$order->id}/show_1_CashInputOrders")}}>
                        ...
                        <th>
                            <a class="btn btn-info btn-sm" href="{{ route('cashbox.cashboxFirst.orderoutput.show', $order) }}">
                                <svg class="c-icon">
                                    <use xlink:href="/assets/icons/coreui/free-symbol-defs.svg#cui-applications"></use>
                                </svg>
                            </a>
                            <a href="{{ route('cashbox.cashboxFirst.orderoutput.edit',$order->id) }}" class="btn btn-primary btn-sm" >
                                <x-svg icon="cui-pencil" />
                            </a>
                            <form style="display:inline" action="{{ route('cashbox.cashboxFirst.orderoutput.destroy',$order->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger btn-sm" type="submit"><x-svg icon="cui-trash" /></button>
                            </form>
                            {{--<a href="{{ route('cashbox.cashboxFirst.orderoutput.destroy',$order->id) }}" class="btn btn-danger btn-sm" >--}}
            ...
            <table id="example" class="table table-striped table-bordered table-cursor" style="width: 100%;">
                ...
                @foreach($ordersInput as $order)
                    <tr data-url={{url("/cashbox/order/{$order->id}/show_1_CashInputOrders")}}>
                        ...
                        <th>{{ $order->patient->f_name }}&nbsp;{{ $order->patient->l_name }}&nbsp;{{ $order->patient->p_name }}</th>
                        <th>
                            <a class="btn btn-info btn-sm" href="{{ route('cashbox.cashboxFirst.orderinput.show', $order->id) }}">
                                <svg class="c-icon">
                                    <use xlink:href="/assets/icons/coreui/free-symbol-defs.svg#cui-applications"></use>
                                </svg>
                            </a>
                            <a href="{{ route('cashbox.cashboxFirst.orderinput.edit',$order->id) }}" class="btn btn-primary btn-sm" >
                                <x-svg icon="cui-pencil" />
                            </a>
                            <form style="display:inline" action="{{ route('cashbox.cashboxFirst.orderinput.destroy',$order->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger btn-sm" type="submit"><x-svg icon="cui-trash" /></button>
                            </form>
