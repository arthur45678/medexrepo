...
class BiochemicalLabN1Controller extends Controller
{
    /**
    ...
    {
        $bl_list = $patient->biochemical_labs_n1()->onlyApproved()->with("attending_doctor")->get();

        return view("samples.biochemical_lab_n1.index")->with(compact("bl_list", "patient"));
    }
    ...
    public function create(Patient $patient)
    {
        ...
        # next_bbe_number
        $biochemical_lab_n1_s_latest = BiochemicalLabN1::latest()->first();
        $last_bbe_number = $biochemical_lab_n1_s_latest->bbe_number ?? 0;
        $next_bbe_number = $last_bbe_number + 1;

        return view("samples.biochemical_lab_n1.create")->with(compact(
            'patient',
            'all_stationary_id',
            'ambulator_id',
            'next_bbe_number'
    ...
    public function store(Request $request, Patient $patient)
    {
        ...
        $patient->biochemical_labs_n1()->create($request->all());
        return response()->json([
            'success' => __('samples.created'),
            'redirect' => route('samples.patients.biochemical-lab-n1.create', $patient),
            'delay' => 2000
        ], 201);
    }
     ...
     * @param  \App\Models\Patient $patient
     * @param  int  $bl_id
     * @param  \App\Models\Samples\BiochemicalLabN1  $bl
     * @return \Illuminate\Http\Response
     */
    public function show(Patient $patient, $bl_id)
    {
        ...
        $bl = $patient->biochemical_labs_n1()->onlyApproved()->findOrFail($bl_id);
        // dd($hr);
        return view("samples.biochemical_lab_n1.show")->with(compact('patient', 'bl', 'ambulator_id', 'all_stationary_id'));
    }
    ...
    public function edit(Patient $patient, $bl_id)
    {
        ...
        $bl = $patient->biochemical_labs_n1()->findOrFail($bl_id);
        $this->authorize("belongs-to-user", $bl);

        return view("samples.biochemical_lab_n1.edit")->with(compact('patient', 'bl', 'ambulator_id', 'all_stationary_id'));
    }
     ...
     *
     * @param  \App\Models\Samples\BiochemicalLabN1  $biochemicalLabN1
     * @return \Illuminate\Http\Response
     */
    public function destroy($patent_id,$id)
    {
        $biochemical_lab = BiochemicalLabN1::where('patient_id',$patent_id)->where('user_id',auth()->id())->find($id);
