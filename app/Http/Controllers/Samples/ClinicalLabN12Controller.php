...
use App\Models\Samples\ClinicalLabN12;
use App\Models\Patient;
use Illuminate\Http\Request;
use App\Models\Approvement;

class ClinicalLabN12Controller extends Controller
{
    /**
    ...
    {
        $cl_list = $patient->clinical_labs_n12()->onlyApproved()->with("attending_doctor")->get();

        return view("samples.clinical_lab_n12.index")->with(compact("cl_list", "patient"));
    }
    ...
    public function create(Patient $patient)
    {
        ...
        # next_bbe_number
        $clinical_labs_n12_s_latest = ClinicalLabN12::latest()->first();
        $last_bbe_number = $clinical_labs_n12_s_latest->bbe_number ?? 0;
        $next_bbe_number = $last_bbe_number + 1;

        return view("samples.clinical_lab_n12.create")->with(compact(
                'patient',
                'all_stationary_id',
                'ambulator_id',
                'next_bbe_number'
    ...
    public function store(Request $request, Patient $patient)
    {
        ...
        $patient->clinical_labs_n12()->create($request->all());
        return response()->json([
            'success' => __('samples.created'),
            'redirect' => route('samples.patients.clinical-lab-n12.create', $patient),
            'delay' => 2000
        ], 201);
    }
    ...
    public function show(Patient $patient, $cl_id)
    {
        ...
        $cl = $patient->clinical_labs_n12()->onlyApproved()->findOrFail($cl_id);
        // dd($hr);
        return view("samples.clinical_lab_n12.show")->with(compact('patient', 'cl', 'ambulator_id', 'all_stationary_id'));
    }
     ...
     * @param  \App\Models\Patient $patient
     * @param  int  $em_id
     * @param  \App\Models\Samples\ClinicalLabN12  $bl
     * @return \Illuminate\Http\Response
     */
    public function edit(Patient $patient, $cl_id)
    {
        ...
        $cl = $patient->clinical_labs_n12()->findOrFail($cl_id);
        $this->authorize("belongs-to-user", $cl);

        return view("samples.clinical_lab_n12.edit")->with(compact('patient', 'cl', 'ambulator_id', 'all_stationary_id'));
    }
     ...
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  \App\Models\Samples\ClinicalLabN12  $cl
     * @return \Illuminate\Http\Response
     */
    ...
    {
        $clinical_lab = ClinicalLabN12::where('patient_id',$patent_id)->where('user_id',auth()->id())->find($id);
